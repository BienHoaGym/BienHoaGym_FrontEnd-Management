    ...
    <v-row class="mb-6 align-center">
        ...
        <div class="d-flex align-center mb-2">
          ...
          <div>
            <h1 class="text-h4 font-weight-bold">
              {{ isStaffViewingOther ? 'Lịch của PT: ' + schedule.userInfo?.fullName : (authStore.canManageSystem ? 'Tổng quan lịch trình hệ thống' : 'Lịch làm việc của tôi') }}
            </h1>
            <p class="text-subtitle-1 text-grey">
              {{ authStore.canManageSystem && !selectedTrainerId ? 'Xem tất cả hoạt động giảng dạy và PT trong toàn hệ thống' : 'Xem các lớp giảng dạy và học viên PT cá nhân' }}
            </p>
          </div>
        </div>
      </v-col>
      <v-col cols="12" md="4">
        <v-autocomplete
          ...
          :items="trainerStore.trainers"
          item-title="fullName"
          item-value="id"
          label="Chọn Huấn luyện viên để xem lịch"
    ...
    <v-window v-model="tab">
      ...
      <v-window-item value="today">
        ...
        <div v-else class="text-center py-16 bg-grey-lighten-5 rounded-xl border-dashed">
          ...
          <p class="text-h6 text-grey mt-4">Không có lớp học nào{{ isStaffViewingOther ? ' của PT này ' : ' ' }}hôm nay.</p>
...
const schedule = ref({ classes: [], personalClients: [], userInfo: null })
...
const isStaffViewingOther = computed(() => !!selectedTrainerId.value && authStore.canManageSystem)
...
const todayClasses = computed(() => {
  const todayEng = dayjs().format('dddd')
...
const fetchSchedule = async () => {
  ...
  try {
    let res
    if (selectedTrainerId.value && authStore.canManageSystem) {
      res = await trainerStore.fetchScheduleById(selectedTrainerId.value)
    } else if (authStore.canManageSystem) {
      // Global view for Admin if no specific PT is selected
      res = await trainerStore.fetchGlobalSchedule()
    } else {
      // For PT, fetch their own schedule
    ...
    if (res.success) {
      ...
      schedule.value = { classes: [], personalClients: [], userInfo: null }
      if (res.message) {
         snack.value = { show: true, message: res.message, color: 'error' }
      }
    }
  } catch (err) {
